...
sub set_layout
{
    ...
    $self->{searchpanel} = $self->FindWindow('searchpanel');
    $self->{itemname} = $self->FindWindow('itemname');
    $self->{lb_itemname} = $self->FindWindow('lb_itemname');
    $self->{lb_hint} = $self->FindWindow('lb_hint');
    $self->{lb_results} = $self->FindWindow('lb_results');
    $self->{mainpanel} = $self->FindWindow('mainpanel');
    $self->{statusbar} = $self->FindWindow('statusbar');
    $self->{bsizer} = $self->FindWindow('bSizer3');
    ...
    my $choices = [  ];
    $self->{itemlist} = Wx::SimpleHtmlListBox->new($self->{searchpanel}, -1, wxDefaultPosition, wxDefaultSize, $choices, wxHLB_DEFAULT_STYLE );
    
    # Add the itemlist to the sizer
    $self->{sizer}->Add($self->{itemlist}, 1, wxALL|wxEXPAND, 5);
    
    # Add sizer to searchpanel
    $self->{searchpanel}->SetSizer($self->{sizer});
    
    # Give the itemname box focus
    $self->{itemname}->SetFocus;
    
    # Set the colors on the widgets
    set_colors($self);
...
sub set_colors
{
	...
	# Set the colors
    $self->{searchpanel}->SetBackgroundColour(Wx::Colour->new("#000000"));
    $self->{searchpanel}->SetForegroundColour(Wx::Colour->new("#000000"));
    $self->{lb_itemname}->SetBackgroundColour(Wx::Colour->new("#000000"));
    $self->{lb_itemname}->SetForegroundColour(Wx::Colour->new("#E8B13F"));
    ...
    $self->{itemlist}->SetBackgroundColour(Wx::Colour->new("#222222"));
    $self->{itemlist}->SetForegroundColour(Wx::Colour->new("#222222"));
...
sub set_events
{
    ...
    EVT_LISTBOX_DCLICK($self, $self->{itemlist}, \&view_item);
...
sub view_item
{
    ...
    my $item = $self->{itemlist}->GetString($self->{itemlist}->GetSelection);
    
    # Remove everything but the id
    $item =~ s/.+ID:&emsp;(\d{1,9})<\/td><\/table><\/font>/$1/;
    
    # Open the item on the grand exchange webpage
    Wx::LaunchDefaultBrowser("http://services.runescape.com/m=itemdb_rs/viewitem.ws?obj=$item");
}
...
sub search_ge
{
    ...
    my $indexes = $self->{itemlist}->GetCount;
    ...
    if ($self->{itemname}->GetValue !~ /^$/)
    {
        ...
        foreach my $item(@result)
        {
            ...
            # Make type font bold and green if F2P item
            $itemdata[2] = "<font color=#088A29>$itemdata[2]</font>" if $itemdata[2] =~ /^Free/;
            # Make type font bold and red if P2P item
            $itemdata[2] = "<font color=#FF8000>$itemdata[2]</font>" if $itemdata[2] =~ /^Members/;

            # Make trend font bold and green if price is going up
            $itemdata[4] = "<b><font color=#088A29>$itemdata[4]</font></b>" if $itemdata[4] =~ /^\+/;
            # Make trend font bold and red if price is going down
            $itemdata[4] = "<b><font color=red>$itemdata[4]</font></b>" if $itemdata[4] =~ /^-/;
            
            # Make price bold and green if price is between 10k and 999k
            $itemdata[3] = "<b><font color=#088A29>$itemdata[3]</font></b>" if $itemdata[3] =~ /k$/;
            # Make price bold and orange if price is between 1m and 999m
            $itemdata[3] = "<b><font color=#FF8000>$itemdata[3]</font></b>" if $itemdata[3] =~ /m$/;
            # Make price bold and black if price is between 1m and 999m
            $itemdata[3] = "<b><font color=#088A85>$itemdata[3]</font></b>" if $itemdata[3] =~ /b\s+$/;
            
            # Add the item to the item list
            $self->{itemlist}->Append("<font color=\"#E8B13F\"><table width=99%><td>Name:&emsp;<b><u>$itemdata[1]</u></b></td><td width=25%>Type:&emsp;$itemdata[2]</td></table><table width=99%><td width=35%>Price:&emsp;$itemdata[3]</td><td width=40%>Today's change(gp):&emsp;$itemdata[4]</td><td width=10%>ID:&emsp;$itemdata[0]</td></table></font>");
        }
