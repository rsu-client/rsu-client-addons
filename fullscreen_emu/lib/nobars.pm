...
{
	# Get the passed data
	my ($offset_left, $offset_top, $offset_right, $offset_bottom) = @_;
	...
	else
	{
		...
		else
		{
			...
			$desk_wh[0] += $offset_right;
			
			# Execute wmctrl to resize the runescape client window
			system "wmctrl -F -r RuneScape -e 0,$offset_left,$offset_top,$desk_wh[0],$desk_wh[1]";
			
			# Return to call with 1
			return 1;
		}
	}
}
